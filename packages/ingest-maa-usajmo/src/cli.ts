...
import { USAJMO_MANIFEST_DIR, loadAllUsajmoManifests } from "./load-manifest";
...
function parseArgs(argv: string[]): { command: CliCommand; flags: BuildPayloadsFlags | ValidateFlags } {
  ...
  let manifestDir = USAJMO_MANIFEST_DIR;
  let outDir = path.resolve(process.cwd(), "out/usajmo-payloads");
...
async function runValidate(flags: ValidateFlags): Promise<number> {
  const results = await loadAllUsajmoManifests({ manifestDir: flags.manifestDir });
  let failed = 0;
  for (const result of results) {
    const label = path.basename(result.manifestPath);
    if (result.ok) {
      console.log(
        `ok  ${label}  USAJMO ${result.data.problemSet.year} (${result.data.problems.length} problems)`
      );
    } else {
      failed += 1;
