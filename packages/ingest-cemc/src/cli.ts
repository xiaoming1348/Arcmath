...
import { EUCLID_MANIFEST_DIR, loadAllEuclidManifests } from "./load-manifest";
...
function parseArgs(argv: string[]): { command: CliCommand; flags: BuildPayloadsFlags | ValidateFlags } {
  ...
  let manifestDir = EUCLID_MANIFEST_DIR;
  let outDir = path.resolve(process.cwd(), "out/euclid-payloads");
...
async function runValidate(flags: ValidateFlags): Promise<number> {
  const results = await loadAllEuclidManifests({ manifestDir: flags.manifestDir });
  let failed = 0;
  for (const result of results) {
    const label = path.basename(result.manifestPath);
    if (result.ok) {
      console.log(`ok  ${label}  (${result.data.problems.length} problems)`);
    } else {
      failed += 1;
...
async function runBuildPayloads(flags: BuildPayloadsFlags): Promise<number> {
  ...
  if (failures.length > 0) {
    console.error(`Refusing to build payloads — ${failures.length} manifest(s) invalid. Run validate-manifests to inspect.`);
    return 1;
  }
