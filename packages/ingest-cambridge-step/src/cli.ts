...
import { STEP_MANIFEST_DIR, loadAllStepManifests } from "./load-manifest";
...
function parseArgs(argv: string[]): { command: CliCommand; flags: BuildPayloadsFlags | ValidateFlags } {
  ...
  let manifestDir = STEP_MANIFEST_DIR;
  let outDir = path.resolve(process.cwd(), "out/step-payloads");
...
async function runValidate(flags: ValidateFlags): Promise<number> {
  const results = await loadAllStepManifests({ manifestDir: flags.manifestDir });
  let failed = 0;
  for (const result of results) {
    const label = path.basename(result.manifestPath);
    if (result.ok) {
      console.log(
        `ok  ${label}  STEP ${result.data.problemSet.exam ?? "?"} ${result.data.problemSet.year} (${result.data.problems.length} problems)`
      );
    } else {
      failed += 1;
...
async function runBuildPayloads(flags: BuildPayloadsFlags): Promise<number> {
  ...
  for (const result of results) {
    ...
    const examSlug = result.data.problemSet.exam ?? "unknown";
    const outName = `${result.data.problemSet.contest.toLowerCase()}-${result.data.problemSet.year}-${examSlug}.json`;
