...
import { MAT_MANIFEST_DIR, loadAllMatManifests } from "./load-manifest";
...
function parseArgs(argv: string[]): { command: CliCommand; flags: BuildPayloadsFlags | ValidateFlags } {
  ...
  let manifestDir = MAT_MANIFEST_DIR;
  let outDir = path.resolve(process.cwd(), "out/mat-payloads");
...
async function runValidate(flags: ValidateFlags): Promise<number> {
  const results = await loadAllMatManifests({ manifestDir: flags.manifestDir });
  let failed = 0;
